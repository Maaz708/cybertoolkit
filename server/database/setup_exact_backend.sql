...
-- Network scans table (matches backend queries)
...
-- File scans table (matches backend queries - uses original_filename)
...
-- Email analyses table (matches backend queries)
...
-- Success message
SELECT 'Exact backend match schema created successfully!' as result;
